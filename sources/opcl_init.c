...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/19 21:44:59 by ahiroko           #+#    #+#             */
/*   Updated: 2019/08/19 21:47:51 by ahiroko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		build_program(t_opcl *opcl)
{
	...
	err = clBuildProgram(opcl->program, 1,
	&opcl->id, "-I includes", NULL, NULL);
	if (CL_SUCCESS != err)
	{
		clGetProgramBuildInfo(opcl->program, opcl->id,
...
static void		check_platform(t_opcl *opcl)
{
	...
	if (CL_SUCCESS != (err |= clGetDeviceIDs(pltfrm,
	CL_DEVICE_TYPE_GPU, nd, &opcl->id, NULL)))
		cl_err_exit(opcl, ERR_INIT_DEVICE);
}
...
void			init_opencl(t_opcl *opcl)
{
	...
	check_platform(opcl);
	if (!(opcl->context = clCreateContext(NULL, 1,
	&opcl->id, NULL, NULL, &err)))
		cl_err_exit(opcl, ERR_INIT_CONTEXT);
	if (!(opcl->queue = clCreateCommandQueue(opcl->context,
	opcl->id, 0, &err)))
		cl_err_exit(opcl, ERR_INIT_COMMAND_QUEUE);
	opcl->out = clCreateBuffer(opcl->context,
	CL_MEM_WRITE_ONLY, WIDTH * HEIGHT * 4, NULL, &err);
	if (CL_SUCCESS != err)
